...
def fix_array_in_string(s, key='bbox_3d": '):
    body = s.split(key)[-1].split("}")[0]
    refine_body = str(parse_float_list(body))
    return s.replace(body, refine_body)


def threedod_process_results(results, norm_coord=1):
    lines = results.strip('\n').strip("```").strip("json").strip("\n").split("\n")
    pred = []
    for line in lines:
        line = line.strip().strip(",")
        line = fix_array_in_string(line)
    ...
    for bbox in pred:
        try:
            pred_bbox = np.array(bbox["bbox_3d"], dtype=float) / float(norm_coord)
...
def main(args):
    ...
    for item in tqdm(data):
        ret = compute_ap(threedod_process_results(item['gt_response']), threedod_process_results(item['pred_response'], 1))
        rets.append(ret)
    threedod_aggregate_results(rets)
